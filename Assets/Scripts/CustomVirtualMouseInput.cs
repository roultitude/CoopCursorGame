...
namespace UnityEngine.InputSystem.UI
{
    ...
    public class CustomVirtualMouseInput : MonoBehaviour
    {
        ...
        private void UpdateMotion()
        {
            if (m_VirtualMouse == null)
                return;
            if (!PlayerManager.Instance.localPlayer) return;
            Vector2 currPos = PlayerManager.Instance.localPlayer.transform.position;
            ...
            else
            {
                ...
                if (Mathf.Approximately(0, m_LastDelta.x) && Mathf.Approximately(0, m_LastDelta.y))
                {
                    // Motion has started.
                ...
                //var delta = new Vector2(m_CursorSpeed * diff.x * deltaTime, m_CursorSpeed * diff.y * deltaTime);
                ...
                Vector2 newPosition = Camera.main.WorldToScreenPoint(PlayerManager.Instance.localPlayer.transform.position);
                
                Vector2 delta = newPosition - m_LastMousePos;
